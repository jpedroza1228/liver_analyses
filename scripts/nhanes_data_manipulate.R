...
pred_matrix[c("albumin_bi", "albumin_cat"), "albumin_g_dl"] <- 0
pred_matrix["albumin_g_dl", c("albumin_bi", "albumin_cat")] <- 0

pred_matrix[c("alp_bi", "alp_cat"), "alp_iu_l"] <- 0
pred_matrix["alp_iu_l", c("alp_bi", "alp_cat")] <- 0


pred_matrix[c("alp_bi", "alp_cat"), "alp_iu_l"] <- 0
pred_matrix["alp_iu_l", c("alp_bi", "alp_cat")] <- 0

pred_matrix[c("ast_bi", "ast_cat"), "ast_u_l"] <- 0
pred_matrix["ast_u_l", c("ast_bi", "ast_cat")] <- 0


pred_matrix[c("alt_bi", "alt_cat"), "alt_u_l"] <- 0
pred_matrix["alt_u_l", c("alt_bi", "alt_cat")] <- 0


pred_matrix["ggt_bi", "ggt_u_l"] <- 0
pred_matrix["ggt_u_l", "ggt_bi"] <- 0


pred_matrix[c("bilirubin_bi", "bilirubin_cat"), "total_bilirubin_mg_dl"] <- 0
pred_matrix["total_bilirubin_mg_dl", c("bilirubin_bi", "bilirubin_cat")] <- 0


pred_matrix["ldl_bi", "ldl_chol_mg_dl"] <- 0
pred_matrix["ldl_chol_mg_dl", "ldl_bi"] <- 0


pred_matrix["hdl_bi", "hdl_chol_mg_dl"] <- 0
pred_matrix["hdl_chol_mg_dl", "hdl_bi"] <- 0


pred_matrix["total_chol_bi", "total_chol_mg_dl"] <- 0
pred_matrix["total_chol_mg_dl", "total_chol_bi"] <- 0


pred_matrix["trigly_bi", "trigly_mg_dl"] <- 0
#pred_matrix["trigly_mg_dl", "trigly_bi"] <- 0


pred_matrix[c("trigly_bi", "trigly_mg_dl", "hdl_bi", "hdl_chol_mg_dl", "ldl_bi", "ldl_chol_mg_dl"), c("total_chol_bi", "total_chol_mg_dl")] <- 0
...
# constant and collinear variables
pred_matrix[, c("told_diabetes", "trigly_bi", "albumin_bi", "alt_bi", "ast_bi")] <- 0
...
model_imp$loggedEvents[1:5, ]

saveRDS(model_imp, here::here("projects/nhanes/imputation_model_more_var_new.rds"))
...
propplot <- function(x, formula, facet = "wrap", ...) {
  ...
  a <- plyr::ddply(meltDF, c(wvars, svars, "variable", "value"), plyr::summarize,
             alc |> count ever45_drink_everyday length(value))
  b <- plyr::ddply(meltDF, c(wvars, svars, "variable"), plyr::summarize,
             tot = length(value))
  mdf <- merge(a,b)
  mdf$prop <- mdf$alc |> count ever45_drink_everyday mdf$tot
